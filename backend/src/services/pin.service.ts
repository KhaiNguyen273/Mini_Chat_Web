...
export const pinMessage = async (conversationId: number, messageId: number, userId: number) => {
  ...
  // MỚI — báo realtime cho cả conversation ai vừa ghim tin nào
  const pinnedByUser = await UserModel.findById(userId);
  try {
    getIO().to(`conversation:${conversationId}`).emit("conversation:pin", {
      conversationId,
      messageId,
      pinnedBy: { id: userId, name: pinnedByUser?.name },
    });
  } catch {
...
export const unpinMessage = async (conversationId: number, messageId: number, userId: number) => {
  ...
  // MỚI — báo realtime cho cả conversation tin nào vừa được bỏ ghim
  try {
    getIO().to(`conversation:${conversationId}`).emit("conversation:unpin", {
      conversationId,
      messageId,
    });
  } catch {
