...
function notificationText(n: Notification): string {
  switch (n.type) {
    ...
    case 'pending_message':
      return `${n.actor?.name || 'Ai đó'} đã nhắn: ${n.preview}`
    case 'new_message':
      return n.preview
    default:
      return n.preview
...
function ProfileNotification() {
  ...
  const handleNotificationClick = (n: Notification) => {
    ...
    if (n.type === 'new_message') {
      ...
      navigate('/contacts')
    } else if (n.type === 'pending_message') {
      navigate('/pending')
    }
  }
  ...
  return (
    <div className="flex flex-col flex-1 h-full bg-[#f7f9fb] overflow-y-auto">
      ...
      <div className="max-w-2xl mx-auto w-full px-6 py-6 flex flex-col gap-6">
              ...
              {requests.map(r => (
                <div key={r.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-[#f7f9fb] transition-colors">
                  ...
                  <div className="flex gap-2 shrink-0">
                    <button onClick={() => acceptRequest(r.id)} className="px-3 py-1.5 rounded-lg bg-[#2563eb] text-white text-xs font-semibold hover:opacity-90 transition-opacity">
                      Chấp nhận
                    </button>
                    <button onClick={() => rejectRequest(r.id)} className="px-3 py-1.5 rounded-lg bg-[#f2f4f6] text-[#565f71] text-xs font-semibold hover:bg-[#e6ebef] transition-colors">
                      Từ chối
                    </button>
                  </div>
                </div>
        ...
        {/* Thông báo chung — tin nhắn mới trong đoạn chat đã có, lời mời/chấp nhận kết bạn */}
        <div className="bg-white rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex flex-col gap-1">
              {notifications.map(n => (
                <div
                  ...
                  className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer hover:bg-[#f7f9fb] transition-colors ${!n.is_read ? 'bg-[#f0f5ff]' : ''}`}
                >
