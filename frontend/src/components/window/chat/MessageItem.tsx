...
function MessageItem({ id, content, isMine, time, avatar, attachments, isPinned, onTogglePin, onDelete, isHighlighted, type, isDeleted, readers, currentUserId }: MessageItemProps) {
  const [showMenu, setShowMenu] = useState(false)
  const menuRef = useRef<HTMLDivElement>(null)
  const confirm = useConfirm()

  const visibleReaders = currentUserId
    ? readers?.filter((r) => String(r.id) !== String(currentUserId))
    : readers
  ...
  const readersRow = visibleReaders && visibleReaders.length > 0 && (
    <div className="flex justify-end gap-0.5 w-full pr-1">
      {visibleReaders.slice(0, 3).map((r) => (
    ...
    return (
      <div className="flex flex-col gap-1 w-full">
        <div className={`flex items-end gap-2 ${isMine ? 'flex-row-reverse' : 'flex-row'}`}>
          ...
          <div className={`max-w-[80%] sm:max-w-xs lg:max-w-md ${isMine ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
  ...
  const showActionMenu = onTogglePin || (isMine && onDelete)
      ...
      <div className={`max-w-[80%] sm:max-w-xs lg:max-w-md ${isMine ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
        ...
        {(content || hasAttachments) && (
          <div className={`relative flex items-center gap-1 ${isMine ? 'flex-row-reverse' : 'flex-row'}`}>
            ...
            <div className={`flex flex-col gap-1.5 rounded-2xl transition-all duration-500 ${
              ...
              {content && (
                <div className={`px-4 py-2.5 rounded-2xl text-sm ${
                  isMine ? 'bg-[#2563eb] text-white rounded-br-sm' : 'bg-[#ecf0f3] text-[#1a1c1e] rounded-bl-sm'
                }`}>
                  {content}
                </div>
              )}
