...
function CreateGroupPopup({ onClose }: CreateGroupPopupProps) {
  ...
  const { friends } = useFriendship() // dùng bạn bè thật thay vì mock
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center">
        <div className="bg-white rounded-2xl shadow-xl w-[480px] max-h-[600px] flex flex-col">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-[#e6ebef]">
            <div className="w-7"/>
            <h2 className="text-base font-bold text-[#1a1c1e]">{step === 'members' ? 'Tạo nhóm' : 'Đặt tên nhóm'}</h2>
            <button onClick={step === 'name' ? () => setStep('members') : onClose} className="w-7 h-7 rounded-full bg-[#f2f4f6] hover:bg-[#e6ebef] flex items-center justify-center transition-colors">
              {step === 'name' ? (
            ...
            <>
              <div className="px-4 py-3 border-b border-[#e6ebef]">
                <div className="flex items-center gap-2 bg-[#f2f4f6] rounded-full px-3 py-2">
                  ...
                  <input type="text" placeholder="Tìm kiếm bạn bè..." value={search} onChange={e => setSearch(e.target.value)} className="flex-1 text-xs bg-transparent outline-none text-[#1a1c1e] placeholder:text-[#70787d]" autoFocus/>
                </div>
              ...
              <div className="overflow-y-auto flex-1 py-2">
                {filtered.map(u => {
                  ...
                  return (
                    <div key={u.id} onClick={() => toggleSelect(u.id)} className="flex items-center gap-3 px-6 py-3 hover:bg-[#f7f9fb] cursor-pointer transition-colors">
                      ...
                      <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${isSelected ? 'bg-[#2563eb] border-[#2563eb]' : 'border-[#70787d]'}`}>
                        {isSelected && <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3"><path d="M20 6 9 17l-5-5"/></svg>}
                      </div>
                    </div>
                  )
                })}
              ...
              <div className="px-6 py-4 border-t border-[#e6ebef]">
                <button onClick={() => selected.length >= 2 && setStep('name')} className={`w-full py-2.5 rounded-xl text-white text-sm font-semibold transition-opacity ${selected.length >= 2 ? 'bg-[#2563eb] hover:opacity-90' : 'bg-[#d7e3ff] cursor-not-allowed'}`}>
            ...
            <>
              <div className="flex flex-col gap-6 px-6 py-6 flex-1">
                <div className="flex flex-col items-center gap-3">
                  <div className="w-20 h-20 rounded-full bg-[#f2f4f6] flex items-center justify-center cursor-pointer hover:bg-[#e6ebef] transition-colors">
                    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#70787d" strokeWidth="1.5"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                  </div>
                ...
                <div>
                  ...
                  <input type="text" placeholder="Nhập tên nhóm..." value={groupName} onChange={e => setGroupName(e.target.value)} autoFocus className="w-full px-3 py-2.5 rounded-lg border border-[#70787d] text-sm text-[#1a1c1e] outline-none focus:border-[#2563eb] transition-colors"/>
                </div>
              ...
              <div className="px-6 py-4 border-t border-[#e6ebef]">
                <button onClick={handleCreate} className={`w-full py-2.5 rounded-xl text-white text-sm font-semibold transition-opacity ${groupName.trim() ? 'bg-[#2563eb] hover:opacity-90' : 'bg-[#d7e3ff] cursor-not-allowed'}`}>
                  Tạo nhóm
                </button>
              </div>
            </>
